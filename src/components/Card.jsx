    ...
    return (
      <div className="grid grid-cols-1 gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 xl:gap-x-12">
          {newsProducts.map((newProduct) => (
            <Link key={newProduct.id} to={""} className="group">
              ...
              <h3 className="mt-6 text-base text-gray-700">{newProduct.name}</h3>
              <p className="mt-2 text-xl font-medium text-gray-900">{newProduct.price}</p>
            </Link>
          ))}
      </div>
    );
